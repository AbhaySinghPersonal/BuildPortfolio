...
from MarketConfig import Smoothing_91,Smoothing_92,Smoothing_12,Smoothing_21,Smoothing_26,MUTUAL_FUND
...
def GetAnalyticalData(Price_Lst,Last_EMA_9,Last_EMA_12,Last_EMA_21,Last_EMA_26,Last_EMA_50,Last_EMA_100,Gain_Lst,Loss_Lst,Is_First_Analysis):
...
def AddAnalyticalDataInMasterFundList(Fund_Grp_Lst):
    ...
    for Fund_Sch in Fund_Grp_Lst:
        ...
        DateTillNoCalFrmtd=GetFutOrPastDate(datetime.datetime.strptime(FundLaunchDt, "%d-%b-%Y"),CAL_FREE_PERIOD,False,"%Y-%m-%d")
        Last_No_Day_Cal=1
...
def RetOneListDesc(Fund_Ind_Row):
   ...
   FundDescSub['FundType']=MUTUAL_FUND
...
def CreateAndLoadAnalyticalData(Fund_Com_Lst,Cur_Dt_Formatted,Fund_Sub):
   ...
   for Fund_Sub_str in Fund_Sub:
      FundCompList=[]
      FundDesc=[]
      FundDescSub,ISIN=RetOneListDesc(Fund_Com_Lst[Fund_Sub_str][-1])
...
def LoadOneFund(Fund_Sub,Cur_Dt_Formatted):
    ...
    CreateAndLoadAnalyticalData(Fund_Com_Lst,Cur_Dt_Formatted,Fund_Sub)
...
def UpdAnalyticDataForOneSch(OneNAVRec,Cur_Dt,Short_Long_Lst):
   ISIN=OneNAVRec[2]
   ...
   if(DateTillNoCalFrmtd>=FundLaunchDate):
      Price_Lst,Last_EMA_9,Last_EMA_12,Last_EMA_21,Last_EMA_26,Last_EMA_50,Last_EMA_100,Gain_Lst,Loss_Lst,SecLast=GDB.GetPrevAnalyticalData(DateTillNoCalFrmtd,ISIN)
   ...
   return Short_Long_Lst

def GetListOfFundsNeedToSell(AllHldng,Short_Long_Lst,ProposeBuySellList):
   ...
   return ProposeBuySellList,CashAvlDueToFndSell
...
def ProposeFundsToAll(Short_Long_Lst):
   ...
   for Usr in UsrLst:
      ...
      ProposeBuySellList,CashAvlDueToFndSell=GetListOfFundsNeedToSell(AllHldngs,Short_Long_Lst,ProposeBuySellList)
      TotCashToDistbute=CashAvlDueToFndSell+Usr.unusedFund
      ProposeBuySellList=DistibuteCashInFunds(Usr.Email,Short_Long_Lst,ProposeBuySellList,TotCashToDistbute)
...
def UpdLatestNAVForAll(LstOfFundNeedsToUpd,Cur_Dt):
   ...
   for OneNAVRec in LstOfFundNeedsToUpd:
      Short_Long_Lst=UpdAnalyticDataForOneSch(OneNAVRec,Cur_Dt,Short_Long_Lst)
      GDB.UpdFILoaded(OneNAVRec[0],OneNAVRec[4])
   Short_Long_Lst=[{'ISIN':'INF0QA701896','UpdDate':'2025-01-09','FundPrice':9.439,'BuySellRecommend':STRONG_LONG},
                   {'ISIN':'INF0QA701383','UpdDate':'2025-01-09','FundPrice':13.821,'BuySellRecommend':STRONG_LONG},
                   {'ISIN':'INF843K01FC8','UpdDate':'2025-01-09','FundPrice':24.4088,'BuySellRecommend':LONG},
                   {'ISIN':'INF579M01910','UpdDate':'2025-01-09','FundPrice':43.75,'BuySellRecommend':SHORT}
                   ]
   if(len(Short_Long_Lst)>0):
      GDB.BulkInsertFIShortLong(Short_Long_Lst)
      ProposeFundsToAll(Short_Long_Lst)
