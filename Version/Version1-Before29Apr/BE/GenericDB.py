    ...
    try:
        ...
        with app.app_context():
            AnalyticalDataRaw = db.session.query(FIAnalyticsData).filter(
                (FIAnalyticsData.ISIN == ISIN) & (FIAnalyticsData.FundPriceAsOnDate > CalPerStrtDt) & (FIAnalyticsData.FundPriceAsOnDate < Cur_Dt)
        ...
        return Price_Lst,Last_EMA_9,Last_EMA_12,Last_EMA_21,Last_EMA_26,Last_EMA_50,Last_EMA_100,Gain_Lst,Loss_Lst,SecLast,Last_FundPrcDt,Last_Sum_Of_Square_Of_Prc_Minus_Mean,Last_Smoothed_Gain,Last_Smoothed_Loss,Last_Signal_Line,PrcandMeanDiffSum,GainCCI,LossCCI
    except IntegrityError as e:
        print(f"IntegrityError occurred: {e}")
    except SQLAlchemyError as e:
        print(f"SQLAlchemyError occurred: {e}")
